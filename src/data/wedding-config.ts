export const weddingConfig = {
  // ——— Измени под себя ———
  bride: "Александра",
  brideGenitive: "Александры",
  groom: "Виктор",
  groomGenitive: "Виктора",
  date: "27 марта 2026",
  time: "12:00",
  venue: "РАГС Петродворцового района",
  address: "Петергоф, Торговая площадь, 5",
  mapLink: "https://yandex.com/maps/-/CPRPfL3g", // ссылка на Яндекс.Карты
  dressCode: "Smart Casual",
  schedule: [
    { time: "11:30", event: "Сбор гостей" },
    { time: "12:00", event: "Церемония" },
    { time: "12:30", event: "Ваши поздравления нам" },
    { time: "13:00", event: "Завершение мероприятия" },
  ],
  // Подарок — ненавязчивая секция
  gift: {
    enabled: true,
    tinkoffCollectUrl: "https://tbank.ru/cf/2Oot7mz50qn",
    heading: "О подарках",
    text: "Лучший подарок для нас — это ваше присутствие и тёплые пожелания. Но если вы хотите сделать нам приятное, мы будем рады вашему вкладу.",
  },

  // Пароль для админ-панели (простая защита)
  adminPassword: "wedding2703.",
};
